...
function __tm_top_message( $format = '%s' ) {
	...
	printf( $format, wp_kses_post( $message ) );
}
...
function __tm_footer_logo() {
	$logo_url = get_theme_mod( 'footer_logo_url', __tm_theme()->customizer->get_default( 'footer_logo_url' ) );
	...
	printf( $logo_format, esc_url( $logo_url ), esc_url( $url ), esc_attr( $alt ), $atts );
}
...
function __tm_footer_copyright() {
	...
	printf( $format, wp_kses_post( __tm_render_macros( $copyright ) ) );
}
...
function __tm_get_site_title_by_type( $type ) {
	...
	$retina_logo     = $retina_logo_url ? sprintf( 'srcset="%s 2x"', esc_url( $retina_logo_url ) ) : '';
...
function __tm_ads( $location ) {
	...
	$format = '<div class="%s-ads">%s</div>';

	printf( $format, esc_attr( $location ), wp_specialchars_decode( $ads, ENT_QUOTES ) );
}
...
/**
 * Display ads for before single post content.
 */
