 ...
 */
function __tm_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->__tm_comment_list_args = $args;
	$comment = $_comment;
	...
	printf( '<%s id="comment-%s" %s>',
		$tag,
		get_comment_ID(),
		comment_class( $args['has_children'] ? 'parent' : '', null, null, false )
	);
 ...
 */
function __tm_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->__tm_comment_list_args['avatar_size'] ) ) {
		$size = $comment->__tm_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( '__tm_comment_author_avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function __tm_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( '__tm_get_comment_author_link', sprintf( '<b class="fn">%s</b>', get_comment_author_link() ), $args );
}
 ...
 */
function __tm_get_comment_date( $args = array() ) {
	$d      = ! empty( $args['d'] ) ? esc_attr( $args['d'] ) : '';
	$format = ! empty( $args['format'] ) ? esc_attr( $args['format'] ) : esc_html__( '%1$s at %2$s', '__tm' );

	$date   = sprintf( $format, get_comment_date( $d ), get_comment_time( $d ) );
	$result = sprintf( '<a href="%3$s" class="comment-date"><time datetime="%1$s" class="comment-date__time">%2$s</time></a>',
		get_comment_time( 'c' ),
		$date,
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( '__tm_get_comment_date', $result, $args );
}
 ...
 */
function __tm_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( '__tm_get_comment_text', $comment_text, $args );
}
 ...
 */
function __tm_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( '__tm_get_comment_reply_link', $reply, $args );
}
 ...
 */
function __tm_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', '__tm' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( '__tm_get_comment_link_edit', $link, $comment->comment_ID, $args );
}
